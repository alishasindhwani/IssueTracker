...
<%
   ...
   Connection conn = null;
	...
	try {
		...
		rset = stmt.executeQuery("select F.iid, F.fid, F.description from Fix F Where F.fid IN (Select fid from Verifies)");
		String fid = request.getParameter("fix");
		...
		if(fid !=null)
		{
			String t = "insert into corrects (fid, correctsdate) values (" +fid + "," + "'" + date + "')";
...
<body>
	...
	<h3>Correct an Issue:</h3>
	<form name="corrects" action="corrects.jsp" method="post">
	<%
			if (rset != null) {
				out.print("<p>Issue <select name=\"fix\">");
				while (rset.next()) {
					int fid = rset.getInt("fid");
					out.print("<option value=\"" + fid + "\">" + "issue #" + rset.getString("iid") + " : "
		...
		%>
		<input type="submit" value="This Fix Corrects the Issue">
	</form>
</body>
</html>